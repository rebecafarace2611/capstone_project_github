...
plt.title("Baseline XGBoost — Precision-Recall Curve (Final Test)")
plt.ylim(0, 0.12)  # zoom: precision is low under 0.42% prevalence
plt.legend(loc="upper right")
plt.tight_layout()
plt.savefig("xgb_pr_curve.png", dpi=200)
plt.close()
...
plt.title("Baseline XGBoost — ROC Curve (Final Test)")
plt.legend(loc="lower right")
plt.tight_layout()
plt.savefig("xgb_roc_curve.png", dpi=200)
plt.close()
...
ax.set_title(f"Baseline XGBoost — Confusion Matrix (q* = {q_star:.5f})")
...
print("  xgb_pr_curve.png")
print("  xgb_roc_curve.png")
print("  xgb_confusion_matrix.png")
print(f"\nSanity check — PR-AUC={pr_auc:.4f}, ROC-AUC={roc_auc:.4f}")
